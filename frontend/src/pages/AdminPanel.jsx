...
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION: Results (Grouped by user)
// ═══════════════════════════════════════════════════════════════════════════════
const ResultsSection = ({ showToast }) => {
  ...
  const load = useCallback(() => {
    ...
    adminService.getResultsGrouped(params)
      .then(r => {
        setGrouped(r.data.users);
  ...
  const handleDelete = async (id, userEmail) => {
    try {
      ...
      setGrouped(prev =>
        prev.map(u => {
          if (u.user_email !== userEmail) return u;
          const updated = { ...u, attempts: u.attempts.filter(a => a.id !== id) };
          updated.attempt_count = updated.attempts.length;
          return updated;
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500">
        {totalUsers} user{totalUsers !== 1 ? "s" : ""} with quiz attempts
      </p>
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <div className="text-center hidden sm:block">
                      <p className="text-lg font-bold text-purple-600">{u.attempt_count}</p>
                      <p className="text-xs text-gray-400">attempt{u.attempt_count !== 1 ? "s" : ""}</p>
                    </div>
                      ...
                      {u.attempts.map(a => (
                        <div key={a.id} className="flex items-center gap-3 bg-gray-50 rounded-xl px-3 py-2.5 text-sm">
                          ...
                          <button
                            ...
                            className="p-1 rounded-lg hover:bg-red-100 text-red-400 flex-shrink-0"
      ...
      {confirm && (
        <Confirm
          message={`Delete result ${confirm.label}? This cannot be undone.`}
          onConfirm={() => handleDelete(confirm.id, confirm.userEmail)}
          onCancel={() => setConfirm(null)}
        />
      )}
    </div>
...
const CollegesSection = ({ showToast }) => {
  ...
  const handleCreate = async () => {
    try {
      ...
      setForm(blankForm);
      load();
    } catch { showToast("Error creating college", "error"); }
  };
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden overflow-x-auto">
        ...
        {colleges.length === 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 text-center py-16">
            ...
            <p className="text-gray-400 text-sm mt-1">Add your first college using the button above</p>
          </div>
        )}
      </div>
