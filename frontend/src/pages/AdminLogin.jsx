...
export default function AdminLogin() {
  ...
  const handleForgot = async (e) => {
    ...
    try {
      ...
      setFpMessage("Reset link sent. Check your inbox (check spam too). Reset link is valid for 30 minutes.");
      setFpEmail("");
    } catch {
        ...
        <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl p-8">
          ...
          {mode === "forgot" ? (
            <form onSubmit={handleForgot} className="space-y-4">
              ...
              {fpMessage && (
                <div className="px-4 py-3 bg-emerald-500/20 border border-emerald-500/40 rounded-xl text-emerald-300 text-sm text-center">
                  {fpMessage}
                </div>
              )}
              ...
              <button
                type="button"
                onClick={() => { setMode("login"); setFpMessage(""); setError(""); }}
                className="w-full text-center text-white/50 text-sm hover:text-white/80 transition-colors"
              >
                ← Back to login
              </button>
            </form>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end -mt-1">
                <button
                  ...
                  className="text-purple-300 text-xs hover:text-purple-100 transition-colors"
                >
                  Forgot password?
                </button>
              </div>
